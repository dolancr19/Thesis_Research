...
function [y, phase_shift_rad] = delay_and_phase_shift_bb_signal(xyz_m,fc_Hz,unit_normal_vector,y0_bb,c_ms,fb_Hz,snr_dB)
...
for ii=(2:numsensors)
    ...
    phase_shift_rad(ii) = tau1_s * (2*pi*fc_Hz);
    %fprintf('ii=%d, phase_shift %f deg\n',ii,(180/pi)*phase_shift_rad);
    
    y_ii = zeros(numpoints1,1);
    n0 = pad_length + delta_N;
    n1 = n0 + numpoints0 - 1;
    y_ii(n0:n1) = y0_bb*exp(sqrt(-1)*phase_shift_rad(ii));
    y(:,ii) = y_ii;
end
